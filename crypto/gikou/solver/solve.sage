...
tmp = sage_encode(P3).encode()+b"\n"
conn.send(tmp)
conn.recvuntil(b"Enter a point: ")
tmp = conn.recvline()
x1,y1 = sage_decode(tmp)
conn.recvuntil(b"Enter a next point: ")
tmp = sage_encode(Q3).encode()+b"\n"
conn.send(tmp)
x2,y2 = sage_decode(conn.recvline())
a1 = a3 = 0
a2 = 6
a4 = ((y2^2-y1^2)-(x2^3-x1^3)-6*(x2^2-x1^2))/(x2-x1)
a6 = y2^2 - (x2^3 + a2*x2^2 + a4*x2)
E1 = EllipticCurve(k,[a1,a2,a3,a4,a6])
P = E1((x1,y1))
Q = E1((x2,y2))
import sys
print("E1:",E1,file=sys.stderr)

dual_isogeny = Isogeny_decomp(Q,3**e3)
